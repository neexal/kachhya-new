...
def save_assignment_files(instance, filename):
        upload_to = 'questions/'
        ext = filename.split('.')[-1]
        # get filename
        if instance.title:
            filename = 'question_files/{}/{}.{}'.format(instance.title,instance.title, ext)
            if os.path.exists(filename):
                new_name = str(instance.title) + str('1')
                filename =  'lesson_question/{}/{}.{}'.format(instance.title,new_name, ext)
        return os.path.join(upload_to, filename)
...
class Assignment(models.Model):
    ...
    file = models.FileField(upload_to=save_assignment_files, null=True, blank=True)
...
def save_assignment_submission_files(instance, filename):
        upload_to = 'answers/'
        ext = filename.split('.')[-1]
        # get filename
        if instance.title:
            filename = 'answer_files/{}/{}.{}'.format(instance.assignemnt,instance.assignment, ext)
            if os.path.exists(filename):
                new_name = str(instance.user) + str('1')
                filename =  'practice_answer/{}/{}.{}'.format(instance.assignment,new_name, ext)
        return os.path.join(upload_to, filename)
...
class Submission(models.Model):
    ...
    description = models.TextField(max_length=1000, null=True)
    file = models.FileField(upload_to="save_assignment_submission_files", null=True, blank=True)
...
# class ReturnAssignment(models.Model):
#     assignment = models.ForeignKey(Submission, on_delete=models.CASCADE)
#     file = models.ForeignObject()
